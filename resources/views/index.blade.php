...
<body class="bg-[#FDFDFC] dark:bg-[#0a0a0a] text-[#1b1b18] dark:text-[#EDEDEC] min-h-screen flex flex-col">
    <header class="w-full px-4 py-4 sm:px-6 lg:px-8 lg:pt-6">
        @if (Route::has('login'))
            <nav class="mx-auto flex max-w-7xl items-center justify-between gap-4">
                ...
                @auth
                    <a href="{{ url('/dashboard') }}"
                        class="inline-flex shrink-0 items-center px-5 py-2 text-sm font-medium text-[#1b1b18] dark:text-[#EDEDEC] border border-[#e3e3e0] dark:border-[#3E3E3A] rounded-full hover:border-[#1915014a] dark:hover:border-[#62605b] transition">
                        Dashboard
                    </a>
                @else
                    <a href="{{ route('login') }}"
                        class="inline-flex shrink-0 items-center px-5 py-2 text-sm font-medium text-[#1b1b18] dark:text-[#EDEDEC] border border-transparent rounded-full bg-white dark:bg-[#161615] shadow-[0px_10px_30px_rgba(0,0,0,0.06)] hover:bg-[#f8f8f6] dark:hover:bg-[#1f1f1d] transition">
                        Sign in
                    </a>
                @endauth
            </nav>
    ...
    <main class="flex-1 w-full px-6 pb-12 lg:px-8">
        <div class="mx-auto grid gap-10 lg:grid-cols-[1.1fr_0.9fr] max-w-7xl items-center">
            <section class="space-y-8">
                <div
                    class="inline-flex items-center gap-2 rounded-full border border-[#e3e3e0] bg-white px-4 py-2 text-sm text-[#706f6c] dark:border-[#3E3E3A] dark:bg-[#161615] dark:text-[#A1A09A] shadow-sm">
                    <span class="h-2 w-2 rounded-full bg-[#10b981]"></span>
                    <span>Built for member and signature card workflows</span>
                </div>

                <div class="space-y-6">
                    <h1 class="text-4xl sm:text-5xl font-semibold tracking-tight leading-tight">Manage staff and
                        signature cards with fast daily access.</h1>
                    <p class="max-w-xl text-base leading-8 text-[#706f6c] dark:text-[#A1A09A]">A modern dashboard for
                        credit union staff to manage teams, add member signature cards, and support quick card access
                        for everyday transactions.</p>
                </div>

                <div class="flex flex-col gap-4 sm:flex-row">
                    <a href="{{ auth()->check() ? url('/dashboard') : route('login') }}"
                        class="inline-flex items-center justify-center rounded-full bg-[#1b1b18] px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-[#00000014] transition hover:bg-[#333232]">
                        {{ auth()->check() ? 'Open dashboard' : 'Sign in to your account' }}
                    </a>
                </div>

                <div class="grid gap-4 sm:grid-cols-3">
                    <div
                        class="rounded-[1.5rem] border border-[#e3e3e0] bg-white p-5 shadow-sm dark:border-[#3E3E3A] dark:bg-[#161615]">
                        <p class="text-sm font-semibold">Secure member profiles</p>
                        <p class="mt-3 text-sm leading-6 text-[#706f6c] dark:text-[#A1A09A]">Keep member details
                            protected and easy to update.</p>
                    </div>
                    <div
                        class="rounded-[1.5rem] border border-[#e3e3e0] bg-white p-5 shadow-sm dark:border-[#3E3E3A] dark:bg-[#161615]">
                        <p class="text-sm font-semibold">Strong password rules</p>
                        <p class="mt-3 text-sm leading-6 text-[#706f6c] dark:text-[#A1A09A]">Enforced validation for
                            safer account creation.</p>
                    </div>
                    <div
                        class="rounded-[1.5rem] border border-[#e3e3e0] bg-white p-5 shadow-sm dark:border-[#3E3E3A] dark:bg-[#161615]">
                        <p class="text-sm font-semibold">Signature card management</p>
                        <p class="mt-3 text-sm leading-6 text-[#706f6c] dark:text-[#A1A09A]">Add member signature cards
                            and keep quick access organized for daily transactions.</p>
                    </div>
                </div>
            </section>

            <aside
                class="relative overflow-hidden rounded-[2rem] border border-[#e3e3e0] bg-gradient-to-br from-[#f7f4ef] to-[#ffffff] p-8 shadow-[0px_30px_80px_rgba(0,0,0,0.08)] dark:border-[#3E3E3A] dark:from-[#121110] dark:to-[#1b1a19]">
                <div
                    class="absolute inset-0 bg-[radial-gradient(circle_at_top_right,_rgba(245,48,3,0.15),_transparent_35%)] pointer-events-none">
                </div>
                <div class="relative flex h-full flex-col justify-between gap-6">
                    <div>
                        <div
                            class="inline-flex items-center rounded-full bg-white/80 px-3 py-1 text-xs uppercase tracking-[0.2em] text-[#706f6c] shadow-sm dark:bg-[#1f1f1d]/80 dark:text-[#A1A09A]">
                            SignatureSuite workflows
                        </div>
                        <h2 class="mt-8 text-3xl font-semibold leading-tight">A confident dashboard for your team</h2>
                        <p class="mt-4 text-sm leading-6 text-[#706f6c] dark:text-[#A1A09A]">Organize staff, members,
                            and signature cards with polished workflows tailored for local credit union branches.</p>
                    </div>

                    <div class="grid gap-4 sm:grid-cols-2">
                        <div
                            class="rounded-3xl bg-white/80 p-5 text-sm text-[#1b1b18] shadow-sm dark:bg-[#161615]/90 dark:text-[#EDEDEC]">
                            <p class="font-semibold">Member onboarding</p>
                            <p class="mt-2 text-[#706f6c] dark:text-[#A1A09A]">Fast creation and validation of new
                                members.</p>
                        </div>
                        <div
                            class="rounded-3xl bg-white/80 p-5 text-sm text-[#1b1b18] shadow-sm dark:bg-[#161615]/90 dark:text-[#EDEDEC]">
                            <p class="font-semibold">Account security</p>
                            <p class="mt-2 text-[#706f6c] dark:text-[#A1A09A]">Strong password requirements and
                                protected access.</p>
                        </div>
                        <div
                            class="rounded-3xl bg-white/80 p-5 text-sm text-[#1b1b18] shadow-sm dark:bg-[#161615]/90 dark:text-[#EDEDEC]">
                            <p class="font-semibold">Signature card management</p>
                            <p class="mt-2 text-[#706f6c] dark:text-[#A1A09A]">Create and manage signature cards for
                                members with a simple staff workflow.</p>
                        </div>
                        <div
                            class="rounded-3xl bg-white/80 p-5 text-sm text-[#1b1b18] shadow-sm dark:bg-[#161615]/90 dark:text-[#EDEDEC]">
                            <p class="font-semibold">Quick card access</p>
                            <p class="mt-2 text-[#706f6c] dark:text-[#A1A09A]">Keep daily transaction access ready with
                                clear card authorization tools.</p>
                        </div>
                    </div>
                </div>
            </aside>
    ...
    <footer class="w-full border-t border-[#e3e3e0] bg-white/60 py-8 dark:border-[#3E3E3A] dark:bg-[#0f0f0e]/60">
        <div class="mx-auto grid max-w-7xl gap-6 px-4 text-sm text-[#706f6c] dark:text-[#A1A09A] sm:px-6 md:grid-cols-[1fr_auto] md:items-end lg:px-8">
            <div>
